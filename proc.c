...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (process = ptable.proc; process < &ptable.proc[NPROC]; process++)
    {
      ...
      listPointer1 = process->holding;
      for (pprocess = ptable.proc; pprocess < &ptable.proc[NPROC]; pprocess++)
      {
        listPointer2 = pprocess->holding;

        for (int ppl = 0; ppl < 16; ppl++)
        {
          lock2 = listPointer2[ppl];
          ...
          {
            lock1 = listPointer1[pl];
            if (lock1 == 0)
              continue;

            if (lock1 == lock2 && pprocess->nice < update)
            {
              update = pprocess->nice;

              if (pprocess->nice < priority)
              {
                priority = pprocess->nice;
              }
            }
          }
        }
